...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./jo")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                try {
                    pp_user = await hisoka.profilePictureUrl(num, 'image')
                } catch {
                ...
                if (anu.action == 'add') {
                  hisoka.sendMessage(anu.id, { image: { url: pp_user }, contextInfo: { mentionedJid: [num] }, caption: `╭━═════════════━• \n│〘Halo @${num.split("@")[0]}〙\n╰━═════════════━• \n╭━═════════════━• \n│⭔ Welcome To\n│⭔ ${metadata.subject}\n╰━═════════════━• \n╭━═════════════━• \n│⭔ THE JO BOT\n│⭔ https://youtube.com/channel/UC-wt99jFVc-zXMkxKRDZ56w\n╰━═════════════━• \n •━══〘𝚃𝙷𝙴𝙹𝙾𝙱𝙾𝚃〙══━•` })
